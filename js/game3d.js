    ...
    for (let i = st.weaponProjectiles.length - 1; i >= 0; i--) {
      ...
      if (p.type === 'deathBolt' && p.isEnemyProjectile) {
        ...
        if (dbdx * dbdx + dbdz * dbdz < p.range * p.range) {
          const boltDmg = damagePlayer(p.damage);
          if (boltDmg > 0) {
  ...
  function tick() {
    ...
    if (!st.paused && !st.gameOver) {
          ...
          if (e.specialAttackState === 'telegraph') {
            ...
            if (e.specialAttackTelegraphTimer <= 0) {
              ...
              if (e.tier >= 10) {
                ...
                if (distSq < 64) { // 8^2 = 64
                  const shockDmg = damagePlayer(20);
                  if (shockDmg > 0) {
      ...
      if (st.autoAttackTimer <= 0 && !st.ghostForm) {
        ...
        if (nearest) {
          const dmg = st.attackDamage * getPlayerDmgMult();
          ...
          if (st.items.crown) {
            ...
            if (second) {
              const dmg2 = st.attackDamage * getPlayerDmgMult();
              damageEnemy(second, dmg2);
