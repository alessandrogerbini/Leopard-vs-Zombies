...
export function launch3DGame(options) {
  ...
  function createEnemy(x, z, baseHp, tier) {
    ...
    return {
      ...
      // BD-220/221: Boss phase system (tier 9: phases 1-3, tier 10: phases 1-4)
      bossPhase: (tier >= 9) ? 1 : 0,
      _lastAttack: null, // BD-220: prevent consecutive repeats
      // BD-221: Titan Charge state machine
      _chargeState: null, // 'telegraph' | 'charging' | 'recovery' | null
      _chargeTimer: 0,
      _chargeDirX: 0,
      _chargeDirZ: 0,
      _chargeDistLeft: 0,
      ...
      _fissureMeshes: [],
      _fissureState: null, // 'telegraph' | 'erupting' | null
      _fissureTimer: 0,
      _fissureData: [], // { startX, startZ, endX, endZ, mesh }
    };
  }
  ...
  /**
   * BD-220/221: Trigger screen shake effect.
   * Stacks additively — a new shake adds to whatever is already running.
   *
   * @param {number} amplitude - Maximum camera offset in world units.
   * @param {number} duration  - Duration in seconds.
   */
  function triggerScreenShake(amplitude, duration) {
    st.screenShakeAmplitude = Math.max(st.screenShakeAmplitude, amplitude);
    st.screenShakeTimer = Math.max(st.screenShakeTimer, duration);
  }
  ...
  function tick() {
    ...
    // BD-220/221: Screen shake offset
    if (st.screenShakeTimer > 0) {
      st.screenShakeTimer -= dt;
      const shakeIntensity = st.screenShakeAmplitude * Math.min(1, st.screenShakeTimer / 0.05);
